...
def get_model(model_name,n_steps_in,n_steps_out):
    ...
    if model_name == 'LSTM':
        # LSTM
        model.add(LSTM(100, activation='sigmoid', return_sequences=True, input_shape=(n_steps_in, 1)))
        model.add(LSTM(100, activation='sigmoid'))
        model.add(Dense(n_steps_out))
    elif model_name == 'BD LSTM':
        ...
        # CNN
        model.add(Conv1D(filters=64, kernel_size=2, activation='relu', input_shape=(n_steps_in, 1)))
        model.add(MaxPooling1D(pool_size=2))
        model.add(Flatten())
        model.add(Dense(20, activation='relu'))
        model.add(Dense(n_steps_out))
    elif model_name == 'Convolutional LSTM':
        # Convolutional LSTM
        model.add(Conv1D(filters=64, kernel_size=2, activation='relu', input_shape=(n_steps_in, 1)))
        ...
        model = create_transformer_model(n_steps_in, n_steps_out, 1, d_model=64,
                                         num_heads=12, ff_dim=64, num_transformer_blocks=3)
    elif model_name == 'MLP':
        # 多层感知机 (MLP)
        model.add(Dense(20, activation='relu', input_shape=(n_steps_in, 1)))
        model.add(Flatten())
        model.add(Dense(20, activation='relu'))
        model.add(Dense(n_steps_out))
    else:
...
def main():
    ...
    # 获取模型
    # model_hub = ['LSTM', 'BD LSTM', 'ED LSTM', 'CNN', 'Convolutional LSTM', 'Transformer', 'MLP']
    model_name='CNN' # 选择模型--------------------------------------------------------------------------------------------------------------------------------
    ...
    columns = ['Round']
    for step in range(1, n_steps_out + 1):
        columns.extend([f'Train_RMSE_Step{step}', f'Test_RMSE_Step{step}', f'Train_MAPE_Step{step}', f'Test_MAPE_Step{step}'])
    ...
    for round in range(rounds):
        ...
        # 训练模型
        train_history = model.fit(x_train,y_train,batch_size=batch_size,shuffle=shuffle,epochs=epochs,verbose=verbose)
